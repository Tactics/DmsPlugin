...
abstract class BaseDmsAspectPropertyTypePeer {
		...
		while($rs->next()) {
			...
			foreach($results as $temp_obj1) {
				$temp_obj2 = $temp_obj1->getDmsAspect(); 				if ($temp_obj2->getPrimaryKey() === $obj2->getPrimaryKey()) {
					$newObject = false;
										$temp_obj2->addDmsAspectPropertyType($obj1); 					break;
				}
			}
			if ($newObject) {
	...
	public static function doSelectJoinAll(Criteria $c, $con = null)
	{
		...
		while($rs->next()) {
			...
			for ($j=0, $resCount=count($results); $j < $resCount; $j++) {
				$temp_obj1 = $results[$j];
				$temp_obj3 = $temp_obj1->getDmsPropertyType(); 				if ($temp_obj3->getPrimaryKey() === $obj3->getPrimaryKey()) {
					$newObject = false;
					$temp_obj3->addDmsAspectPropertyType($obj1); 					break;
				}
			}
	...
	public static function doInsert($values, $con = null)
	{
		...
		$criteria->remove(DmsAspectPropertyTypePeer::ID); 
	...
	public static function doDeleteAll($con = null)
	{
	 ...
	 public static function doDelete($values, $con = null)
	 {
		...
		if ($values instanceof Criteria) {
			$criteria = clone $values; 		} elseif ($values instanceof DmsAspectPropertyType) {
	...
	public static function doValidate(DmsAspectPropertyType $obj, $cols = null)
	{
		...
		$res =  BasePeer::doValidate(DmsAspectPropertyTypePeer::DATABASE_NAME, DmsAspectPropertyTypePeer::TABLE_NAME, $columns);
    if ($res !== true) {
        $request = sfContext::getInstance()->getRequest();
        foreach ($res as $failed) {
            $col = DmsAspectPropertyTypePeer::translateFieldname($failed->getColumn(), BasePeer::TYPE_COLNAME, BasePeer::TYPE_PHPNAME);
